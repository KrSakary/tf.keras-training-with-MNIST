...
def model_simpledense(input_shape):
    ...
    conv2d_0 = tf.keras.layers.Conv2D(filters=128, kernel_size=(4, 4), strides=(1, 1), padding='VALID')(inputs)
    bn_0 = tf.keras.layers.BatchNormalization()(conv2d_0)
    relu_0 = tf.keras.layers.ReLU()(bn_0)

    conv2d_1 = tf.keras.layers.Conv2D(filters=64, kernel_size=(4, 4), strides=(1, 1), padding='VALID')(relu_0)
    bn_1 = tf.keras.layers.BatchNormalization()(conv2d_1)
    relu_1 = tf.keras.layers.ReLU()(bn_1)

    flat_2 = tf.keras.layers.Flatten()(relu_1)
    dense_2 = tf.keras.layers.Dense(units=256)(flat_2)
    bn_2 = tf.keras.layers.BatchNormalization()(dense_2)
    relu_2 = tf.keras.layers.ReLU()(bn_2)

    dense_1 = tf.keras.layers.Dense(units=10)(relu_2)
...
simpledense = model_simpledense(X.shape[1:])
logits = simpledense(X)
...
with tf.Session() as sess:
    ...
    for epoch in range(100):
        ...
        for batch in range(0, data_num_train, batch_size):
            l, acc, _ = sess.run((loss, accuracy, optimizer), feed_dict={X: x_train[batch:batch + batch_size],
